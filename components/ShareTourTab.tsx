          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-sm animate-fade-in">
            <div className="bg-white p-8 rounded-[2rem] shadow-2xl border border-white w-full max-w-md relative overflow-hidden">
                ...
                <div className="flex justify-end gap-3 mt-8">
                    ...
                    <button onClick={handleAddAgency} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-blue-500/30 hover:bg-blue-700 transition-all uppercase tracking-wide">প্রোফাইল তৈরি করুন</button>
                </div>
            </div>
          </div>
      )}
                        ...
                        <div className="bg-white rounded-[1.75rem] border border-slate-200 overflow-hidden shadow-sm">
                                ...
                                <div className="p-6 bg-slate-50 border-b border-slate-100 animate-fade-in">
                                    <div className="grid grid-cols-2 gap-4 mb-4">
                                        <div className="col-span-2">
                                            <label className="text-[9px] font-bold text-slate-400 uppercase ml-1">গেস্ট নাম</label>
                                            <input className="w-full border border-slate-200 bg-white p-3 rounded-xl text-sm outline-none font-semibold focus:ring-2 focus:ring-violet-500" placeholder="নাম" value={newBooking.name} onChange={e => setNewBooking({...newBooking, name: e.target.value})} />
                                        </div>
                                        <div className="col-span-2">
                                            <label className="text-[9px] font-bold text-slate-400 uppercase ml-1">মোবাইল</label>
                                            <input className="w-full border border-slate-200 bg-white p-3 rounded-xl text-sm outline-none font-semibold focus:ring-2 focus:ring-violet-500" placeholder="ফোন" value={newBooking.phone} onChange={e => setNewBooking({...newBooking, phone: e.target.value})} />
                                        </div>
                                        <div>
                                            <label className="text-[9px] font-bold text-slate-400 uppercase ml-1">সিট</label>
                                    ...
                                    <div className="flex justify-between items-center bg-white p-3 rounded-xl border border-slate-200">
                                        <div className="text-xs font-bold text-slate-500">
                                            মোট বাকি: <span className="text-violet-600 text-sm">৳{((parseInt(newBooking.seatCount)||0) * (parseInt(newBooking.unitPrice)||0)).toLocaleString()}</span>
                                        </div>
                            ...
                            {!agency.guests || agency.guests.length === 0 ? (
                                <div className="text-center py-12 flex flex-col items-center justify-center">
                                    ...
                                    <p className="text-xs text-slate-400 italic font-medium">কোন গেস্ট নেই।</p>
                                </div>
                            ) : (
                                            ...
                                            {isEditing && (
                                                <div className="mt-4 p-4 bg-white border border-slate-200 rounded-xl shadow-lg relative z-20 animate-fade-in">
                                                    ...
                                                    <div className="grid grid-cols-3 gap-3">
                                                        <div>
                                                            <label className="text-[9px] font-bold text-slate-400 block mb-1 uppercase">রেগুলার</label>
                                                        ...
                                                        <div>
                                                            <label className="text-[9px] font-bold text-slate-400 block mb-1 uppercase">ডিস ১</label>
                                                            <input type="number" min="0" value={seatBreakdown.disc1} onChange={(e) => setSeatBreakdown({...seatBreakdown, disc1: safeNum(e.target.value)})} 
                                                                className="w-full p-2.5 border border-slate-200 rounded-lg text-sm font-bold text-center outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-200" />
                                                        </div>
                                                        <div>
                                                            <label className="text-[9px] font-bold text-slate-400 block mb-1 uppercase">ডিস ২</label>
